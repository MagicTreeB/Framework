# MagicTree.Framework.CodeGenerator

Cryptographically secure code generation package with configurable length and character sets.

## Overview

`MagicTree.Framework.CodeGenerator` provides a robust service for generating various types of codes (verification codes, tokens, coupon codes, API keys, etc.) with configurable lengths and character sets. Uses `RandomNumberGenerator` for cryptographic security.
...
```xml
<ItemGroup>
    <ProjectReference Include="..\..\Core\MagicTree.Framework.CodeGenerator\MagicTree.Framework.CodeGenerator.csproj" />
</ItemGroup>
```
...
```csharp
using MagicTree.Framework.CodeGenerator.Enums;
