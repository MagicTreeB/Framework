...
Works alongside other MMO.Core packages:

| Package | Purpose | Middleware Order |
|---------|---------|------------------|
| **MMO.Core.RateLimit** | Request throttling | 1st (before routing) |
| **MMO.Core.Idempotency** | Duplicate prevention | 2nd (before routing) |
| **MMO.Core.Middlewares** (NullResponse) | Null handling | 3rd (after routing) |
| **MMO.Core.Middlewares** (JwtMiddleware) | JWT parsing | 4th (after auth) |
| **MMO.Core.Middlewares** (AutoSaveChanges) | EF Core auto-save | 5th (before endpoints) |
| **MMO.Core.Exceptions** | Global exception handler | After CORS, before auth |
| **MMO.Core.HybridCache** | Query result caching | N/A (service, not middleware) |
