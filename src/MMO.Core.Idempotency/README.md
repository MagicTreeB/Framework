...
```xml
<ProjectReference Include="..\..\..\Core\MMO.Core.Idempotency\MMO.Core.Idempotency.csproj" />
```
...
```csharp
using MMO.Core.Idempotency.Extensions;
...
The package automatically detects if `IConnectionMultiplexer` is already registered (e.g., by `MMO.Core.RateLimit`) and reuses the connection:
