...
Add reference to `MMO.Core.RabbitMQ` in your API project:

```xml
<ItemGroup>
  <ProjectReference Include="..\..\Core\MMO.Core.RabbitMQ\MMO.Core.RabbitMQ.csproj" />
</ItemGroup>
```
...
```csharp
using MMO.Core.RabbitMQ.Extensions;
...
```csharp
using MMO.Core.RabbitMQ.Interfaces;
