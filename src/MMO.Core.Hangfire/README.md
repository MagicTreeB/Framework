...
`MMO.Core.Hangfire` provides a standardized wrapper around Hangfire for reliable background job execution across all microservices. Supports fire-and-forget, delayed, recurring, and continuation jobs with SQL Server or in-memory storage.
...
```xml
<ItemGroup>
    <ProjectReference Include="..\..\Core\MMO.Core.Hangfire\MMO.Core.Hangfire.csproj" />
</ItemGroup>
```
...
- Documentation: See `Core/MMO.Core.Hangfire/README.md`
